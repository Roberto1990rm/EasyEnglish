...
<div class="text-center mb-6">
    @if(auth()->user()->image)
        <img src="{{ asset('storage/' . auth()->user()->image) }}" alt="Foto de perfil" class="mx-auto w-42 h-42 object-cover rounded-full shadow-md mb-2">
...
<form action="{{ route('profile.update') }}" method="POST" enctype="multipart/form-data" class="bg-gray-50 p-4 rounded shadow-md space-y-4">
    @csrf
    ...
    <div>
        ...
        <input type="password" name="password" class="w-full border-gray-300 rounded mt-1" placeholder="••••••">
    </div>
    <div>
        <label class="block text-sm font-semibold">Confirmar contraseña</label>
        <input type="password" name="password_confirmation" class="w-full border-gray-300 rounded mt-1" placeholder="••••••">
    </div>
